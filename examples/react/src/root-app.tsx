    ...
    <div style={containerStyle}>
      <div style={cardStyle}>
        <div style={headerStyle}>
          <img src={context.user?.avatarUrl} alt={context.user?.name} style={avatarStyle} />
          <div>
        ...
        <div style={{ marginTop: theme.spacingLg }}>
          ...
          <div style={{ display: 'flex', gap: theme.spacingMd, alignItems: 'center', marginTop: theme.spacingSm }}>
            <div style={{ fontWeight: 600 }}>{context.org?.name}</div>
            <span style={badgeStyle}>{context.org?.plan}</span>
          </div>
        </div>

        <div style={{ marginTop: theme.spacingLg, display: 'flex', gap: theme.spacingMd }}>
          <button style={buttonStyle} onClick={handleApiCall}>Make API Call</button>
          <a href="#" style={{ alignSelf: 'center', ...mutedStyle, textDecoration: 'underline' }}>Learn more</a>
        </div>
      </div>
    </div>
  );
